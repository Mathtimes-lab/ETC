...
class Kiwoom(QMainWindow):
    ...
    def _log_buy_trade(self, code, stock_name, buy_date, buy_time, target_raw, buy_price, slippage):
        ...
        if os.path.exists(self.CSV_FILE_NAME):
            try:
                ...
                # [수정] 기존 파일의 헷갈리는 컬럼명을 '목표가(보정X)'로 자동 치환 (하위 호환성)
                if '5%상승가(보정X)' in df_exist.columns:
                    df_exist = df_exist.rename(columns={'5%상승가(보정X)': '목표가(보정X)'})
        ...
        # [수정] 헷갈리는 컬럼명을 '목표가(보정X)'로 직관적으로 변경
        new_data = {
            '종목코드': f"'{code}",
            '종목명': stock_name,
            '매수일': buy_date,
            '매수시간': buy_time,
            '목표가(보정X)': int(target_raw),
            '실제매입가': buy_price,
    ...
    def _log_sell_trade(self, code, stock_name, sell_date, sell_time, sell_price):
        ...
        try:
            ...
            # [수정] 기존 파일 읽을 때 컬럼명 강제 치환 (하위 호환성)
            if '5%상승가(보정X)' in df.columns:
                df = df.rename(columns={'5%상승가(보정X)': '목표가(보정X)'})
        ...
        if rqname == "미체결요청":
            ...
            if os.path.exists(self.CSV_FILE_NAME):
                try:
                    ...
                    # [수정] 엑셀 로딩 시 컬럼명 강제 치환 (하위 호환성)
                    if '5%상승가(보정X)' in history_df.columns:
                        history_df = history_df.rename(columns={'5%상승가(보정X)': '목표가(보정X)'})
                ...
                if qty > 0:
                    ...
                    if not history_df.empty:
                        ...
                        if not history_df.loc[mask].empty:
                            ...
                            try:
                                # [수정] 변경된 컬럼명으로 데이터 로드
                                target_raw = int(history_df.loc[last_idx, '목표가(보정X)'])
                                ...
                                csv_slippage = float(history_df.loc[last_idx, '슬리피지(%)'])
                            except Exception:
                                pass
