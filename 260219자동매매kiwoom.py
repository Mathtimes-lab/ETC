...
class Kiwoom(QMainWindow):
    ...
    def _get_stock_info(self, code):
        ...
        self.kiwoom.dynamicCall("CommRqData(QString, QString, int, QString)", "주식기본정보요청", "opt10001", 0, "0104")
    ...
    def _handler_tr_data(self, scr_no, rqname, trcode, recordname, prev_next, data_len, err_code, msg, splm_msg):
        if rqname == "미체결요청":
            ...
            for i in range(cnt):
                ...
                order_no = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, rqname, i, "주문번호").strip()
                order_type = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, rqname, i, "주문구분").strip()
                if "매수" in order_type:
            ...
            for i in range(cnt):
                ...
                price = int(self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, rqname, i, "매입가").strip())
                
                if qty > 0:
                    # 초기 잔고 로딩 시에는 전일종가 기반 추정값 사용
                    ...
                    self.held_stocks[code] = {
                        'qty': qty, 'price': price, 'buy_date': '2026-02-19', 'buy_time': '09:00:00', 
            ...
            # [신규] 시가, 전일종가 수신
            open_price = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, rqname, 0, "시가").strip()
            prev_close = self.kiwoom.dynamicCall("GetCommData(QString, QString, int, QString)", trcode, rqname, 0, "전일종가").strip()
            
            self.tr_data_temp = {
                'open': abs(int(open_price)),
                'prev': abs(int(prev_close))
            }
    ...
    def _execute_buy(self, code):
        ...
        # [수정] TR 요청으로 시가, 전일종가 획득 (정확한 목표가 계산용)
        try:
            info = self._get_stock_info(code)
            prev_price = info.get('prev', 0)
        ...
        # [수정] 목표가 계산 로직: MAX(전일종가 * 1.05, 당일 시가)
    ...
    def after_login(self): self.login_event_loop.exit()
        ...
        if ret == 1:
            ...
            for c in conditions:
                ...
                if name in [self.BUY_STRATEGY_NAME, self.SELL_STRATEGY_NAME]:
                    ...
                    # 딜레이 없이 유지
                    self.kiwoom.dynamicCall("SendCondition(QString, QString, int, int)", "0156" if "매수" in name else "0157", name, int(idx), 0)
    ...
    def _handler_chejan_data(self, gubun, item_cnt, fid_list):
        if gubun == '0': # 접수/체결
            ...
            if "매수" in order_type:
                if status == "접수": self.open_buy_orders[code] = order_no
                elif status == "체결":
            ...
            if qty > 0:
                if code in self.held_stocks: self.held_stocks[code]['qty'] = qty
                else: self.held_stocks[code] = {'qty': qty, 'price': 0}
            else:
    ...
    def _print_slippage_report(self):
        ...
        for code, info in self.held_stocks.items():
            ...
            print(f"{name:<10} | {info.get('buy_date','')} {info.get('buy_time','')} | {int(target):<10} | {info['price']:<10} | {slippage:.2f}%")
